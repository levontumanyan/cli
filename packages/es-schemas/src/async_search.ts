...
export const AsyncSearchSubmitRequest = z.object({
  index: z.lazy(() => Indices).describe('A comma-separated list of index names to search; use `_all` or empty string to perform the operation on all indices').optional().meta({ found_in: 'path' }),
  wait_for_completion_timeout: z.lazy(() => Duration).describe('Blocks and waits until the search is completed up to a certain timeout. When the async search completes within the timeout, the response won’t include the ID as the results are not stored in the cluster.').optional().meta({ found_in: 'query' }),
  keep_alive: z.lazy(() => Duration).describe('Specifies how long the async search needs to be available. Ongoing async searches and any saved search results are deleted after this period.').optional().meta({ found_in: 'query' }),
  ...
  batched_reduce_size: z.lazy(() => long).describe('Affects how often partial results become available, which happens whenever shard results are reduced. A partial reduction is performed every time the coordinating node has received a certain number of new shard responses (5 by default).').optional().meta({ found_in: 'query' }),
  ...
  max_concurrent_shard_requests: z.lazy(() => integer).describe('The number of concurrent shard requests per node this search executes concurrently. This value should be used to limit the impact of the search on the cluster in order to limit the number of concurrent shard requests').optional().meta({ found_in: 'query' }),
  ...
  suggest_mode: z.lazy(() => SuggestMode).describe('Specify suggest mode').optional().meta({ found_in: 'query' }),
  suggest_size: z.lazy(() => long).describe('How many suggestions to return in response').optional().meta({ found_in: 'query' }),
  ...
  _source_excludes: z.lazy(() => Fields).describe('A list of fields to exclude from the returned _source field').optional().meta({ found_in: 'query' }),
  _source_includes: z.lazy(() => Fields).describe('A list of fields to extract and return from the _source field').optional().meta({ found_in: 'query' }),
