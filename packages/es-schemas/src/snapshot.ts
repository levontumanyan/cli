...
import { AcknowledgedResponseBase, ByteSize, DateTime, Duration, DurationValue, EpochTime, ErrorCause, ExpandWildcards, Id, IndexName, Indices, Metadata, Name, Names, ShardStatistics, SortOrder, Uuid, VersionNumber, VersionString, double, integer, long } from './_types.ts'
...
export const SnapshotRepositorySettingsBase = z.object({
  ...
  max_restore_bytes_per_sec: z.lazy(() => ByteSize).describe('The maximum snapshot restore rate per node. It defaults to unlimited. Note that restores are also throttled through recovery settings.').optional(),
  max_snapshot_bytes_per_sec: z.lazy(() => ByteSize).describe('The maximum snapshot creation rate per node. It defaults to 40mb per second. Note that if the recovery settings for managed services are set, then it defaults to unlimited, and the rate is additionally throttled through recovery settings.').optional()
...
export const SnapshotAzureRepositorySettings = z.object({
  ...
  delete_objects_max_size: z.lazy(() => integer).describe('The maxmimum batch size, between 1 and 256, used for `BlobBatch` requests. Defaults to 256 which is the maximum number supported by the Azure blob batch API.').optional(),
...
export const SnapshotIndexDetails = z.object({
  shard_count: z.lazy(() => integer),
  size: z.lazy(() => ByteSize).optional(),
  size_in_bytes: z.lazy(() => long),
  max_segments_per_shard: z.lazy(() => long)
...
export const SnapshotReadOnlyUrlRepositorySettings = z.object({
  ...SnapshotRepositorySettingsBase.shape,
  http_max_retries: z.lazy(() => integer).describe('The maximum number of retries for HTTP and HTTPS URLs.').optional(),
  http_socket_timeout: z.lazy(() => Duration).describe('The maximum wait time for data transfers over a connection.').optional(),
  max_number_of_snapshots: z.lazy(() => integer).describe('The maximum number of snapshots the repository can contain. The default is `Integer.MAX_VALUE`, which is 2^31-1 or `2147483647`.').optional(),
...
export const SnapshotS3RepositorySettings = z.object({
  ...
  delete_objects_max_size: z.lazy(() => integer).describe('The maxmimum batch size, between 1 and 1000, used for `DeleteObjects` requests. Defaults to 1000 which is the maximum number supported by the  AWS DeleteObjects API.').optional(),
  get_register_retry_delay: z.lazy(() => Duration).describe('The time to wait before trying again if an attempt to read a linearizable register fails.').optional(),
  max_multipart_parts: z.lazy(() => integer).describe('The maximum number of parts that Elasticsearch will write during a multipart upload of a single object. Files which are larger than `buffer_size × max_multipart_parts` will be chunked into several smaller objects. Elasticsearch may also split a file across multiple objects to satisfy other constraints such as the `chunk_size` limit. Defaults to `10000` which is the maximum number of parts in a multipart upload in AWS S3.').optional(),
  max_multipart_upload_cleanup_size: z.lazy(() => integer).describe('The maximum number of possibly-dangling multipart uploads to clean up in each batch of snapshot deletions. Defaults to 1000 which is the maximum number supported by the AWS ListMultipartUploads API. If set to `0`, Elasticsearch will not attempt to clean up dangling multipart uploads.').optional(),
  readonly: z.boolean().describe('If true, the repository is read-only. The cluster can retrieve and restore snapshots from the repository but not write to the repository or create snapshots in it. Only a cluster with write access can create snapshots in the repository. All other clusters connected to the repository should have the `readonly` parameter set to `true`. If `false`, the cluster can write to the repository and create snapshots in it. IMPORTANT: If you register the same snapshot repository with multiple clusters, only one cluster should have write access to the repository. Having multiple clusters write to the repository at the same time risks corrupting the contents of the repository.').optional(),
  ...
  'throttled_delete_retry.delay_increment': z.lazy(() => Duration).describe('The delay before the first retry and the amount the delay is incremented by on each subsequent retry. The default is 50ms and the minimum is 0ms.').optional(),
  'throttled_delete_retry.maximum_delay': z.lazy(() => Duration).describe('The upper bound on how long the delays between retries will grow to. The default is 5s and the minimum is 0ms.').optional(),
  'throttled_delete_retry.maximum_number_of_retries': z.lazy(() => integer).describe('The number times to retry a throttled snapshot deletion. The default is 10 and the minimum value is 0 which will disable retries altogether. Note that if retries are enabled in the Azure client, each of these retries comprises that many client-level retries.').optional()
...
export const SnapshotShardsStats = z.object({
  done: z.lazy(() => long).describe('The number of shards that initialized, started, and finalized successfully.'),
  failed: z.lazy(() => long).describe('The number of shards that failed to be included in the snapshot.'),
  finalizing: z.lazy(() => long).describe('The number of shards that are finalizing but are not done.'),
  initializing: z.lazy(() => long).describe('The number of shards that are still initializing.'),
  started: z.lazy(() => long).describe('The number of shards that have started but are not finalized.'),
  total: z.lazy(() => long).describe('The total number of shards included in the snapshot.')
...
export const SnapshotSnapshotStats = z.object({
  ...
  start_time_in_millis: z.lazy(() => EpochTime).describe('The time, in milliseconds, when the snapshot creation process started.'),
  time: z.lazy(() => Duration).optional(),
  time_in_millis: z.lazy(() => DurationValue).describe('The total time, in milliseconds, that it took for the snapshot process to complete.'),
...
export const SnapshotSnapshotShardFailure = z.object({
  ...
  node_id: z.lazy(() => Id).optional(),
  reason: z.string(),
  shard_id: z.lazy(() => integer),
  index_uuid: z.lazy(() => Id),
  status: z.string()
...
export const SnapshotSnapshotInfo = z.object({
  ...
  duration: z.lazy(() => Duration).optional(),
  duration_in_millis: z.lazy(() => DurationValue).optional(),
  end_time: z.lazy(() => DateTime).optional(),
  end_time_in_millis: z.lazy(() => EpochTime).optional(),
  ...
  index_details: z.record(z.lazy(() => IndexName), SnapshotIndexDetails).optional(),
  metadata: z.lazy(() => Metadata).optional(),
  reason: z.string().optional(),
  repository: z.lazy(() => Name).optional(),
  snapshot: z.lazy(() => Name),
  shards: z.lazy(() => ShardStatistics).optional(),
  start_time: z.lazy(() => DateTime).optional(),
  start_time_in_millis: z.lazy(() => EpochTime).optional(),
  state: z.string().optional(),
  uuid: z.lazy(() => Uuid),
  version: z.lazy(() => VersionString).optional(),
  version_id: z.lazy(() => VersionNumber).optional(),
...
export const SnapshotStatus = z.object({
  ...
  uuid: z.lazy(() => Uuid).describe('The universally unique identifier (UUID) for the snapshot.')
...
export const SnapshotRepositoryVerifyIntegrityRequest = z.object({
  name: z.lazy(() => Names).describe('The name of the snapshot repository.').meta({ found_in: 'path' }),
  blob_thread_pool_concurrency: z.lazy(() => integer).describe('If `verify_blob_contents` is `true`, this parameter specifies how many blobs to verify at once.').optional().meta({ found_in: 'query' }),
  index_snapshot_verification_concurrency: z.lazy(() => integer).describe('The maximum number of index snapshots to verify concurrently within each index verification.').optional().meta({ found_in: 'query' }),
  index_verification_concurrency: z.lazy(() => integer).describe('The number of indices to verify concurrently. The default behavior is to use the entire `snapshot_meta` thread pool.').optional().meta({ found_in: 'query' }),
  ...
  max_failed_shard_snapshots: z.lazy(() => integer).describe('The number of shard snapshot failures to track during integrity verification, in order to avoid excessive resource usage. If your repository contains more than this number of shard snapshot failures, the verification will fail.').optional().meta({ found_in: 'query' }),
  meta_thread_pool_concurrency: z.lazy(() => integer).describe('The maximum number of snapshot metadata operations to run concurrently. The default behavior is to use at most half of the `snapshot_meta` thread pool at once.').optional().meta({ found_in: 'query' }),
  snapshot_verification_concurrency: z.lazy(() => integer).describe('The number of snapshots to verify concurrently. The default behavior is to use at most half of the `snapshot_meta` thread pool at once.').optional().meta({ found_in: 'query' }),
...
export const SnapshotStatusRequest = z.object({
  repository: z.lazy(() => Name).describe('The snapshot repository name used to limit the request. It supports wildcards (`*`) if `<snapshot>` isn\'t specified.').optional().meta({ found_in: 'path' }),
  snapshot: z.lazy(() => Names).describe('A comma-separated list of snapshots to retrieve status for. The default is currently running snapshots. Wildcards (`*`) are not supported.').optional().meta({ found_in: 'path' }),
...
export const SnapshotVerifyRepositoryCompactNodeInfo = z.object({
  name: z.lazy(() => Name).describe('A human-readable name for the node. You can set this name using the `node.name` property in `elasticsearch.yml`. The default value is the machine\'s hostname.')
...
export const SnapshotVerifyRepositoryRequest = z.object({
  name: z.lazy(() => Name).describe('The name of the snapshot repository to verify.').meta({ found_in: 'path' }),
  master_timeout: z.lazy(() => Duration).describe('The period to wait for the master node. If the master node is not available before the timeout expires, the request fails and returns an error. To indicate that the request should never timeout, set it to `-1`.').optional().meta({ found_in: 'query' }),
  timeout: z.lazy(() => Duration).describe('The period to wait for a response from all relevant nodes in the cluster after updating the cluster metadata. If no response is received before the timeout expires, the cluster metadata update still applies but the response will indicate that it was not completely acknowledged. To indicate that the request should never timeout, set it to `-1`.').optional().meta({ found_in: 'query' })
