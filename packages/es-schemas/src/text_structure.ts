...
export const TextStructureFindFieldStructureRequest = z.object({
  ...
  documents_to_sample: z.lazy(() => uint).describe('The number of documents to include in the structural analysis. The minimum value is 2.').optional().meta({ found_in: 'query' }),
  ...
  field: z.lazy(() => Field).describe('The field that should be analyzed.').meta({ found_in: 'query' }),
  ...
  index: z.lazy(() => IndexName).describe('The name of the index that contains the analyzed field.').meta({ found_in: 'query' }),
...
export const TextStructureFindStructureRequest = z.object({
  ...
  should_parse_recursively: z.boolean().describe('If the format is `ndjson`, you can specify whether to parse nested JSON objects recursively. The nested objects are parsed to a maximum depth equal to the default value of the `index.mapping.depth.limit` setting. Anything beyond that depth is parsed as an `object` type field. For formats other than `ndjson`, this parameter is ignored.').optional().meta({ found_in: 'query' }),
  timeout: z.lazy(() => Duration).describe('The maximum amount of time that the structure analysis can take. If the analysis is still running when the timeout expires then it will be stopped.').optional().meta({ found_in: 'query' }),
...
export const TextStructureFindStructureResponse = z.object({
  ...
  field_stats: z.record(z.lazy(() => Field), TextStructureFieldStat).describe('The most common values of each field, plus basic numeric statistics for the numeric `page_count` field. This information may provide clues that the data needs to be cleaned or transformed prior to use by other Elastic Stack functionality.'),
  ...
  num_messages_analyzed: z.lazy(() => integer).describe('The number of distinct messages the lines contained. For NDJSON, this value is the same as `num_lines_analyzed`. For other text formats, messages can span several lines.'),
  ...
  num_lines_analyzed: z.lazy(() => integer).describe('The number of lines of the text that were analyzed.'),
