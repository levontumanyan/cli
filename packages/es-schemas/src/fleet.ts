...
export const FleetGlobalCheckpointsRequest = z.object({
  index: z.union([z.lazy(() => IndexName), z.lazy(() => IndexAlias)]).describe('A single index or index alias that resolves to a single index.').meta({ found_in: 'path' }),
...
export const FleetMsearchRequest = z.object({
  index: z.union([z.lazy(() => IndexName), z.lazy(() => IndexAlias)]).describe('A single target to search. If the target is an index alias, it must resolve to a single index.').optional().meta({ found_in: 'path' }),
...
export const FleetSearchRequest = z.object({
  index: z.union([z.lazy(() => IndexName), z.lazy(() => IndexAlias)]).describe('A single target to search. If the target is an index alias, it must resolve to a single index.').meta({ found_in: 'path' }),
  allow_no_indices: z.boolean().describe('A setting that does two separate checks on the index expression. If `false`, the request returns an error (1) if any wildcard expression (including `_all` and `*`) resolves to zero matching indices or (2) if the complete set of resolved indices, aliases or data streams is empty after all expressions are evaluated. If `true`, index expressions that resolve to no indices are allowed and the request returns an empty result.').optional().meta({ found_in: 'query' }),
