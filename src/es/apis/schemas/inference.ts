...
import { Duration, Id, StreamResult, byte, double, float, integer, long } from './_types.ts'
...
export const InferenceAnthropicTaskSettings = z.object({
  max_tokens: z.lazy(() => integer).describe('For a `completion` task, it is the maximum number of tokens to generate before stopping.'),
  temperature: z.lazy(() => float).describe('For a `completion` task, it is the amount of randomness injected into the response. For more details about the supported range, refer to Anthropic documentation.').optional(),
  top_k: z.lazy(() => integer).describe('For a `completion` task, it specifies to only sample from the top K options for each subsequent token. It is recommended for advanced use cases only. You usually only need to use `temperature`.').optional(),
  top_p: z.lazy(() => float).describe('For a `completion` task, it specifies to use Anthropic\'s nucleus sampling. In nucleus sampling, Anthropic computes the cumulative distribution over all the options for each subsequent token in decreasing probability order and cuts it off once it reaches the specified probability. You should either alter `temperature` or `top_p`, but not both. It is recommended for advanced use cases only. You usually only need to use `temperature`.').optional()
...
export const InferenceBaseReasoningDetail = z.object({
  ...
  index: z.lazy(() => integer).describe('The index of the reasoning detail, which indicates its position in the sequence of reasoning details generated by the model.').optional()
...
export const InferenceOpenAIServiceSettings = z.object({
  ...
  dimensions: z.lazy(() => integer).describe('The number of dimensions the resulting output embeddings should have. It is supported only in `text-embedding-3` and later models. If it is not set, the OpenAI defined default for the model is used.').optional(),
