...
export const ClusterHealthRequest = z.object({
  ...
  wait_for_active_shards: z.lazy(() => WaitForActiveShards).describe('Wait for the specified number of active shards. Use `all` to wait for all shards in the cluster to be active. Use `0` to not wait.').optional().meta({ found_in: 'query' }),
  wait_for_events: z.lazy(() => WaitForEvents).describe('Wait until all currently queued events with the given priority are processed.').optional().meta({ found_in: 'query' }),
...
export const ClusterInfoResponse = z.object({
  ...
  http: z.lazy(() => NodesHttp).optional(),
...
export const ClusterPutComponentTemplateRequest = z.object({
  ...
  master_timeout: z.lazy(() => Duration).describe('Period to wait for a connection to the master node. If no response is received before the timeout expires, the request fails and returns an error.').optional().meta({ found_in: 'query' }),
  template: z.lazy(() => IndicesPutIndexTemplateIndexTemplateMapping).describe('The template to be applied which includes mappings, settings, or aliases configuration.').meta({ found_in: 'body' }),
...
export const ClusterStatsDenseVectorOffHeapStats = z.object({
  ...
  total_cenif_size_bytes: z.lazy(() => long),
  total_cenif_size: z.lazy(() => ByteSize).optional(),
