...
export const AggregationsRandomSamplerAggregation = z.object({
  ...
  shard_seed: integer.describe('When combined with seed, setting shard_seed ensures 100% consistent sampling over shards where data is exactly the same.').optional()
...
export interface AggregationsAggregationContainerShape {
  ...
  random_sampler?: AggregationsRandomSamplerAggregation | undefined
...
export const SearchSearchRequestBody = z.object({
  ...
  rank: RankContainer.describe('The Reciprocal Rank Fusion (RRF) to use.').optional(),
...
const QueryDslFunctionScoreContainerCommonProps = z.object({
  ...
  _name: z.string().describe('A name to identify which function matched and influenced the score.').optional()
})
...
export const SecurityRoleDescriptor = z.object({
  ...
  remote_indices: z.array(SecurityRemoteIndicesPrivileges).describe('A list of indices permissions for remote clusters.').optional(),
  remote_cluster: z.array(SecurityRemoteClusterPrivileges).describe('A list of cluster permissions for remote clusters. NOTE: This is limited a subset of the cluster permissions.').optional(),
  global: z.union([z.array(SecurityGlobalPrivilege), SecurityGlobalPrivilege]).describe('An object defining global privileges. A global privilege is a form of cluster privilege that is request-aware. Support for global privileges is currently limited to the management of application privileges.').optional(),
...
export const SecurityApiKey = z.object({
  ...
  certificate_identity: z.string().describe('The certificate identity associated with a cross-cluster API key. Restricts the API key to connections authenticated by a specific TLS certificate. Only applicable to cross-cluster API keys.').optional(),
