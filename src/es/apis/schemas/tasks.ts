...
export const TasksCancelRequest = z.object({
  task_id: z.lazy(() => TaskId).describe('The task identifier.').optional().meta({ found_in: 'path' }),
...
export const TasksListRequest = z.object({
  ...
  group_by: z.lazy(() => TasksGroupBy).describe('A key that is used to group tasks in the response. The task lists can be grouped either by nodes or by parent tasks.').optional().meta({ found_in: 'query' }),
