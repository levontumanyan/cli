...
export const NodesClient = z.object({
  id: z.lazy(() => long).describe('Unique ID for the HTTP client.').optional(),
  ...
  opened_time_millis: z.lazy(() => long).describe('Time at which the client opened the connection.').optional(),
  closed_time_millis: z.lazy(() => long).describe('Time at which the client closed the connection if the connection is closed.').optional(),
  last_request_time_millis: z.lazy(() => long).describe('Time of the most recent request from this client.').optional(),
...
export const NodesProcessor = z.object({
  count: z.lazy(() => long).describe('Number of documents transformed by the processor.').optional(),
...
export const NodesIngestStats = z.object({
  count: z.lazy(() => long).describe('Total number of documents ingested during the lifetime of this node.'),
...
export const NodesThreadCount = z.object({
  active: z.lazy(() => long).describe('Number of active threads in the thread pool.').optional(),
  completed: z.lazy(() => long).describe('Number of tasks completed by the thread pool executor.').optional(),
  largest: z.lazy(() => long).describe('Highest number of active threads in the thread pool.').optional(),
...
export const NodesStats = z.object({
  ...
  http: z.lazy(() => NodesHttp).describe('HTTP connection information.').optional(),
  ingest: z.lazy(() => NodesIngest).describe('Statistics about ingest preprocessing.').optional(),
  ...
  script: z.lazy(() => NodesScripting).describe('Contains script statistics for the node.').optional(),
  script_cache: z.record(z.string(), z.union([NodesScriptCache, z.array(NodesScriptCache)])).optional(),
  thread_pool: z.record(z.string(), z.lazy(() => NodesThreadCount)).describe('Statistics about each thread pool, including current size, queue and rejected tasks.').optional(),
  timestamp: z.lazy(() => long).optional(),
...
export const NodesStatsRequest = z.object({
  ...
  level: z.lazy(() => NodeStatsLevel).describe('Indicates whether statistics are aggregated at the node, indices, or shards level.').optional().meta({ found_in: 'query' }),
  timeout: z.lazy(() => Duration).describe('Period to wait for a response. If no response is received before the timeout expires, the request fails and returns an error.').optional().meta({ found_in: 'query' }),
