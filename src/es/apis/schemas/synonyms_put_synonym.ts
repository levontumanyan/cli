...
export const SynonymsPutSynonymRequest = z.object({
  ...
  refresh: z.boolean().describe('If `true`, the request will refresh the analyzers with the new synonyms set and wait for the new synonyms to be available before returning. If `false`, analyzers will not be reloaded with the new synonym set').optional().meta({ found_in: 'query' }),
