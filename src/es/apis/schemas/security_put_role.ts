...
export const SecurityPutRoleRequest = z.object({
  ...
  global: z.record(z.string(), z.any()).describe('An object defining global privileges. A global privilege is a form of cluster privilege that is request-aware. Support for global privileges is currently limited to the management of application privileges.').optional().meta({ found_in: 'body' }),
  indices: z.array(SecurityIndicesPrivileges).describe('A list of indices permissions entries.').optional().meta({ found_in: 'body' }),
  remote_indices: z.array(SecurityRemoteIndicesPrivileges).describe('A list of remote indices permissions entries. NOTE: Remote indices are effective for remote clusters configured with the API key based model. They have no effect for remote clusters configured with the certificate based model.').optional().meta({ found_in: 'body' }),
  remote_cluster: z.array(SecurityRemoteClusterPrivileges).describe('A list of remote cluster permissions entries.').optional().meta({ found_in: 'body' }),
