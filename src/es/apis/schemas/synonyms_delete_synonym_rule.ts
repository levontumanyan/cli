...
export const SynonymsDeleteSynonymRuleRequest = z.object({
  ...
  refresh: z.boolean().describe('If `true`, the request will refresh the analyzers with the deleted synonym rule and wait for the new synonyms to be available before returning. If `false`, analyzers will not be reloaded with the deleted synonym rule').optional().meta({ found_in: 'query' })
